...
** If the operation has a single argument, we return (1). This means the read was 'succesfull'.
** We read the encoding byte and put the type of arguments in a int[4].
** int[i] == 1 > register, int[i] == 2 > direct, int[i] == 3 > indirect.
** We match the type of arguments against the possible arguments types for the opcode.
...
/*
** READ_OPERATION
** Read_and_check_encoding will check if encodingbyte matches opcode. If not, 
** the return is (0) and the operation is not going to be executed and we skip to next operations.
** Check_and_save_arguments will read the arguments from memory and store them in struct operation.
** If an argument is incorrect, the return is (0) and the operation will also not be executed. 
** If the arguments are CORRECT it will return (1) with the proper arguments set into struct operation.
*/
